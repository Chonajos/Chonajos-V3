...
INSERT INTO menu (id_menu,descripcion,tipo, nivel, url_sistema)  values (s_menu.nextVal, 'Razón Social', 0,'10.9','/views/catRazonSocial.xhtml');
...
INSERT INTO menu (id_menu,descripcion,tipo, nivel, url_sistema)  values (s_menu.nextVal, 'Ventas Cliente', 0,'50.3','/views/reporteClienVentUti.xhtml');
...
INSERT INTO menu (id_menu,descripcion,tipo, nivel, url_sistema)  values (s_menu.nextVal, 'Historial Crediticio', 0,'80.6','/views/historialCredito.xhtml');
...
CREATE TABLE CREDITO(
    ...
    CONSTRAINT c_CREDITO_ID_SUCURSAL_fk FOREIGN KEY(ID_SUCURSAL_FK) references SUCURSAL(ID_SUCURSAL_PK)
    );
...
ID_HORARIO_PK NUMBER NOT NULL,
ID_USUARIO_FK NUMBER,
HORA_ENTRADA DATE,
HORA_SALIDA DATE,
FECHA_INICIO DATE,
FECHA_FIN DATE,
TOLERANCIA VARCHAR(6),
CONSTRAINT C_ID_HORARIO_PK PRIMARY KEY(ID_HORARIO_PK),
CONSTRAINT C_HU_ID_UDUARIO_FK FOREIGN KEY(ID_USUARIO_FK) REFERENCES USUARIO(ID_USUARIO_PK)
);


CREATE SEQUENCE S_HORARIO_USUARIO
INCREMENT BY 1
START WITH 1
MINVALUE 0;

CREATE TABLE DIA_DESCANSO_USUARIO(
ID_DIA_DESCANSO_PK NUMBER NOT NULL,
ID_USUARIO_FK NUMBER,
FECHA_INICIO DATE,
FECHA_FIN DATE,
DIA VARCHAR(13),
CONSTRAINT C_DDU_DIA_DESCANCO_PK PRIMARY KEY (ID_DIA_DESCANSO_PK),
CONSTRAINT C_DDC_ID_USUARIO_FK FOREIGN KEY (ID_USUARIO_FK) REFERENCES USUARIO(ID_USUARIO_PK)
);

CREATE SEQUENCE S_DIA_DESCANSO_USUARIO
INCREMENT BY 1
START WITH 1
MINVALUE 0;
COMMENT ON COLUMN DIA_DESCANSO_USUARIO.DIA IS 'SE PUEDEN AGREGAR VARIOS DIAS DE LA SEMANA SEPARADAOS POR UNA COMO EJEMPLO 0,1 ESTO DICE QUE EL RECURSO TIENE COMO DIAS DE DESCANSO DOMINGO Y LUNES';


CREATE TABLE COMPROBANTES_DIGITALES(
ID_COMPROBANTES_DIGITALES_PK NUMBER NOT NULL,
ID_TIPO_FK NUMBER,
ID_LLAVE_FK NUMBER,
FECHA DATE,
NOMBRE VARCHAR(13),
FICHERO BLOB
CONSTRAINT C_ID_COMPROBANTES_DIGITALES_PK PRIMARY KEY (ID_COMPROBANTES_DIGITALES_PK)
);

CREATE SEQUENCE S_COMPROBANTES_DIGITALES
INCREMENT BY 1
START WITH 1
MINVALUE 0;
--TIPOS--
--1 ENTRADA MERCANCIA MAYOREO
...
ID_USUARIO_FK NUMBER,
ID_STATUS_FK NUMBER,
NOMBRE_FACTURA_TIMBRADA VARCHAR(200),
RFC_EMISOR VARCHAR,
CADENA_ORIGINAL VARCHAR(600),
IMPORTE NUMBER(14,2),
...
ID_DATOS_FACTURACION_PK NUMBER NOT NULL,
ID_CLIENTE_FK NUMBER,
ID_SUCURSAL_FK NUMBER,
...
REGIMEN VARCHAR(50),
FIELD VARCHAR(60),
RUTA_LLAVE_PRIVADA VARCHAR(200),
RUTA_CERTIFICADO VARCHAR(200),
RUTA_LLAVE_PRIVADA_CANCEL VARCHAR(200),
RUTA_CERTIFICADO_CANCEL VARCHAR(200),

CONSTRAINT C_DF_ID_DATOS_FACTURACION_PK PRIMARY KEY (ID_DATOS_FACTURACION_PK),
CONSTRAINT C_DF_ID_CLIENTE_FK FOREIGN KEY (ID_CLIENTE_FK) REFERENCES CLIENTE(ID_CLIENTE),
CONSTRAINT c_DF_ID_CP_FISCAL FOREIGN KEY(ID_CP_FISCAL) references CODIGOS_POSTALES(ID_PK),
CONSTRAINT C_DF_ID_SUCURSAL_FK FOREIGN KEY (ID_SUCURSAL_FK) REFERENCES SUCURSAL(ID_SUCURSAL_PK)
);

CREATE SEQUENCE S_DATOS_FACTURACION
INCREMENT BY 1
START WITH 1
MINVALUE 0;
...
ID_CATALOGO_SAT_PK NUMBER NOT NULL,
CODIGO VARCHAR(10),
DESCRIPCION VARCHAR(150),
TIPO NUMBER,
CONSTRAINT C_CS_ID_CATALOGO_SAT_PK PRIMARY KEY (ID_CATALOGO_SAT_PK)
);

CREATE SEQUENCE S_CATALOGO_SAT
INCREMENT BY 1
START WITH 1
MINVALUE 0;
...
ID_PRODUCTO_FACTURADO_PK NUMBER NOT NULL,
ID_TIPO_LLAVE_FK NUMBER,
ID_LLAVE_FK NUMBER,
...
CANTIDAD NUMBER,
KILOS NUMBER,
CONSTRAINT C_PF_ID_PRODUCTO_FACTURADO_PK PRIMARY KEY (ID_PRODUCTO_FACTURADO_PK),
CONSTRAINT C_PF_ID_FACTURA_FK FOREIGN KEY (ID_FACTURA_FK) REFERENCES FACTURAS(ID_FACTURA_PK)
);


CREATE SEQUENCE S_PRODUCTO_FACTURADO
INCREMENT BY 1
START WITH 1
MINVALUE 0;


--EXPIRA CONTRASEÑA EN SQL
