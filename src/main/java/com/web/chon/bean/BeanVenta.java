...
//        System.out.println("clinete ticket" + cliente.toString());
        ...
        if (!data.getIdTipoVentaFk().equals(new BigDecimal(1))) {
            ...
            paramReport.put("totalCompraDescuento", "$"+df.format(c.getMontoCredito().toString()));
            paramReport.put("totalDescuentoLetra", totalVentaDescuentoStr);
            paramReport.put("aCuenta", "$"+df.format(dejaACuenta.toString()));
            paramReport.put("descuentoVenta", "$"+df.format(descuento.toString()));
        }
    }
    ...
    public void generateReport(int idVenta, int folioVenta) {
        ...
        try {
            ...
            if (data.getIdTipoVentaFk().equals(new BigDecimal(1))) {
                ...
                System.out.println("reporte de credito ticketCredito");
