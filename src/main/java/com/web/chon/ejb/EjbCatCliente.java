...
public class EjbCatCliente implements NegocioCatCliente {
    ...
    public List<Object[]> getReporteClienteVentasUtilidad(BigDecimal idCliente, String fechaInicio, String fechaFin) {
        try {
            StringBuffer sQuery = new StringBuffer("SELECT CLI.NOMBRE||' '|| CLI.APELLIDO_PATERNO ||' '|| CLI.APELLIDO_MATERNO AS NOMBRE_COMPLETO,CLI.ID_CLIENTE, "
            ...
            sQuery.append("GROUP BY CLI.NOMBRE||' '|| CLI.APELLIDO_PATERNO ||' '|| CLI.APELLIDO_MATERNO,CLI.ID_CLIENTE "
                    + " ORDER BY DIAS_RECUPERACION,(TOTAL_MENUDEO_CONTADO+TOTAL_MENUDEO_CREDITO+TOTAL_MAYOREO_CONTADO+TOTAL_MAYOREO_CREDITO) DESC ");
